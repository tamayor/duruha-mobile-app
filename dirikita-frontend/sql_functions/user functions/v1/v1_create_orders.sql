...
END LOOP;
END IF;
-- Parse listing_ids (optional)
grp_listing_ids := ARRAY []::uuid [];
IF jsonb_typeof(grp->'listing_ids') = 'array' THEN FOR v_jidx IN 0..jsonb_array_length(grp->'listing_ids') - 1 LOOP v_lid := grp->'listing_ids'->>v_jidx;
IF v_lid IS NOT NULL
AND v_lid <> '' THEN grp_listing_ids := array_append(grp_listing_ids, v_lid::uuid);
END IF;
END LOOP;
END IF;
...
WHERE pv.produce_id = or_produce_id
    AND (
        grp_is_any
        OR fo.variety_id = ANY(grp_variety_ids)
    )
...
-- ------------------------------------------------------------
IF NOT grp_is_any THEN FOR v_variety_idx IN 1..cardinality(grp_variety_ids) LOOP -- Use selected listing price if available, else variety base price.
v_selected_listing_id := NULL;
IF v_variety_idx <= cardinality(grp_listing_ids) THEN v_selected_listing_id := grp_listing_ids [v_variety_idx];
END IF;
IF v_selected_listing_id IS NOT NULL THEN
SELECT pvl.duruha_to_consumer_price INTO v_variety_price
FROM public.produce_variety_listing pvl
WHERE pvl.listing_id = v_selected_listing_id;
ELSE
SELECT pv.price INTO v_variety_price
FROM public.produce_varieties pv
WHERE pv.variety_id = grp_variety_ids [v_variety_idx];
END IF;
v_prices := array_append(v_prices, COALESCE(v_variety_price, 0.0));
v_qty_taken := array_append(v_qty_taken, 0.0);
END LOOP;
END IF;
-- ------------------------------------------------------------
...
END IF;
ELSE any_variety_pos := array_position(any_variety_ids, var_rec.variety_id);
...
-- C2 = far
remaining := remaining - qty_to_take;
any_variety_pos := array_position(any_variety_ids, var_rec.variety_id);
IF any_variety_pos IS NULL THEN any_variety_ids := array_append(any_variety_ids, var_rec.variety_id);
any_qty_taken := array_append(any_qty_taken, qty_to_take);
any_prices := array_append(
    any_prices,
...
-- INSERT consumer_orders (quality_fee = computed amount, not rate).
...
END IF;
-- STEP 3 — RESOLVE CONSUMER LOCATION (shared across all produce lines)
-- Read the geography column value directly from public.users.
-- The location column stores EWKB hex (e.g. 0101000020E6100000...).
-- We carry it as geography so every downstream call that needs it
-- (delivery fee density check, carrier search) can use it without
-- any float extraction or point reconstruction.
SELECT u.location::geography INTO v_consumer_location
FROM public.user_consumers uc
    JOIN public.users u ON u.id = uc.user_id
WHERE uc.consumer_id = (payload->0->>'consumer_id')
LIMIT 1;
orders := payload;
...
or_consumer_in := (ord->>'consumer_id')::text;
...
idx;
END IF;
IF or_consumer_in IS NULL THEN RAISE EXCEPTION 'order[%]: consumer_id is required',
idx;
END IF;
...
idx;
END IF;
-- Enforce single-consumer constraint.
IF batch_consumer_id IS NULL THEN batch_consumer_id := or_consumer_in;
ELSIF batch_consumer_id <> or_consumer_in THEN RAISE EXCEPTION 'order[%]: all orders must belong to the same consumer ' '(expected %, got %)',
idx,
batch_consumer_id,
or_consumer_in;
END IF;
SELECT uc.user_id::uuid INTO or_consumer_user_id
FROM public.user_consumers uc
WHERE uc.consumer_id = or_consumer_in;
IF or_consumer_user_id IS NULL THEN RAISE EXCEPTION 'order[%]: consumer not found: %',
idx,
or_consumer_in;
END IF;
IF NOT EXISTS (
