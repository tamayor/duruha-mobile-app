...
ELSE -- ── specific varieties: find min/max among selected variety IDs ──
SELECT MIN(pvl.duruha_to_consumer_price),
    MAX(pvl.duruha_to_consumer_price) INTO v_any_min_price,
    v_any_max_price
FROM public.produce_variety_listing pvl
...
VALUES (
        v_consumer_id,
        p_note,
        COALESCE(v_payment_method, 'Cash')::public.payment_method,
        true
    )
...
-- 6c. One covg per date inside this cop
v_item_index := 0;
FOR v_date_needed IN
SELECT (d.value#>>'{}')::date
FROM jsonb_array_elements(v_dates) AS d(value) LOOP -- 6d. Insert covg — one per date
...
-- 6e. consumer_orders_variety insertion — two cases:
--   is_any:   insert ALL varieties for this produce + form
--   specific: insert only the given variety_ids
IF v_is_any THEN -- ── Case 1: is_any — all varieties for this produce + form ──
FOR v_variety_id IN
