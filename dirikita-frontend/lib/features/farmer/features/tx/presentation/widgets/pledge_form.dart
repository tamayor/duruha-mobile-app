...
import 'demand_calendar_picker.dart';
...
class _PledgeFormState extends State<PledgeForm> {
          ...
          builder: (context) {
            ...
            return DuruhaDateInput(
              ...
              onTap: () async {
                await showDialog(
                  context: context,
                  builder: (context) => DemandCalendarPicker(
                    cropId: widget.produce.id,
                    selectedDates: widget.state.selectedHarvestDates,
                    onDatesChanged: (newDates, demandMap) {
                      widget.onDatesChanged(newDates, demandMap);
                    },
                  ),
                );
                // Refresh simple UI display
                setState(() {});
              },
            );
          },
  ...
  Future<void> _showPerDateDialog(
    ...
    if (demandData != null && demandData.varietyBreakdown.isNotEmpty) {
      ...
      // Fetch fresh
      final repo = TransactionDemandRepository();
      try {
